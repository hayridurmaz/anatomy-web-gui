...
class ImageRow extends React.Component<IProps & ReduxProps> {
    ...
    saveImage = (event, mediaId: number) => {
        ...
        if (this.state.chosenSystem === this.props.item.system.id && JSON.stringify(this.state.chosenTopics) === JSON.stringify(defVals)) {
    ...
    render() {
        ...
        if (this.state.modify) {
            topicInfo =
                (
                    <Dropdown placeholder='Topic' style={{ marginLeft: 20 }} defaultValue={this.state.chosenTopics} multiple selection options={this.props.topics} onChange={(event, data) => { this.handleTopic(data.value); }} />
                )
        } else {
            topicInfo =
                (
        ...
        if (this.state.modify) {
            systemInfo =
                (
                    <Dropdown placeholder='System' style={{ marginLeft: 20, heigth: 10 }} defaultValue={this.state.chosenSystem} selection options={this.props.systems} onChange={(event, data) => { this.handleSystem(data.value); }} />
                )
        } else {
            systemInfo =
                (
        ...
        if (this.state.modify) {
            buttons = (
                <div>
                    <Button basic style={{ marginLeft: 20 }} color='red' content='Cancel' onClick={() => { this.cancelModify() }} />
                    <Button basic style={{ marginLeft: 20 }} color='green' content='Save Image' disabled={this.state.disabledSaveButton} onClick={() => { this.saveImage(event, this.props.item.id) }} />
                </div>
            )
        } else {
            buttons = (
                <div>
                    <Button basic style={{ marginLeft: 20 }} color='red' content='Delete Image' onClick={() => { this.deleteImage(this.props.item.id) }} />
                    <Button basic style={{ marginLeft: 20 }} color='orange' content='Modify Image' onClick={() => { this.modifyImage() }} />
                </div>
            )
        }
